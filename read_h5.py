    ...
    for file_name in h5_files:
        ...
        with h5py.File(file_path, 'r') as f:
            ...
            if 'timestamps' in f and 'heart_rate' in f:
                ...
                existing_timestamps = {entry['timestamp'] for entry in existing_data.values()} if existing_data else set()
                ...
                for i in range(len(timestamps)):
                    ...
                    try:
                        ...
                        # If the timestamp is numeric (e.g., Unix timestamp), you can use fromtimestamp()
                        if timestamp_str.isdigit():
                            timestamp_dt = datetime.fromtimestamp(int(timestamp_str))  # Convert Unix timestamp
                        else:
                            # If it's a string date, parse it accordingly
                    ...
                    if date_str in existing_timestamps:
                        print(f"Data for timestamp {timestamp_str} already exists. Skipping.")
                        continue   
                    
                    data_entry = {
                        'timestamp': timestamp_str,
                        'heart_rate': float(heart_rate[i]),  # Convert heart_rate to native Python float
                        'date': date_str
                    }

                    # Push new data to Firebase under the user's node
                    user_ref.push(data_entry)
                    print(f"Uploaded data entry for timestamp {data_entry['timestamp']}.")
            else:
                print(f"Error: Missing required datasets in {file_name}")
